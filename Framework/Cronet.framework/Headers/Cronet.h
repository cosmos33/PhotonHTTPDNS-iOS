...
typedef void (^RequestCompletionBlock)(NSURLRequest* request,NSError *error);

typedef void (^ModifyRequestBlock)(NSMutableURLRequest* request);

typedef NSArray<NSString*> *(^HttpDNSBlock)(NSString* host);
...
/// intermediate certificates or theszzzzzzxxxxxx end-entry certificate. Authentication will
...
+ (Class)getCRNHTTPProtocolHandler;

+ (void)setHttpDNSBlock:(HttpDNSBlock)block;
